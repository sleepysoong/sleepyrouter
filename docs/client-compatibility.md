...
| 서피스 | 엔드포인트 | 소스 앵커 | 테스트 |
| --- | --- | --- | --- |
| 헬스체크 | `GET /health` | [server.go](../internal/srv/server.go) | `server_test.go` |
| OpenAI 모델 | `GET /v1/models` | [server.go](../internal/srv/server.go), [catalog.go](../internal/providers/catalog.go) | `server_test.go`, `openrouter_test.go`, `nvidia_test.go` |
| OpenAI 채팅 | `POST /v1/chat/completions` | [server.go](../internal/srv/server.go), [sseutil.go](../internal/sseutil/sseutil.go) | `server_test.go` |
| Anthropic 메시지 | `POST /anthropic/v1/messages`, `POST /anthropic/messages` | [server.go](../internal/srv/server.go), [translate.go](../internal/srv/translate.go) | `server_test.go`, `translate_test.go` |
| Anthropic 토큰 카운트 | `POST /anthropic/v1/messages/count_tokens`, `POST /anthropic/messages/count_tokens` | [server.go](../internal/srv/server.go) | `server_test.go` |
...
3. `server.go`, `translate.go`, `sseutil.go`에서 엔드포인트, 번역, SSE 동작을 확인하세요.
