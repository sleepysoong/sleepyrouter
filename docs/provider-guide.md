...
- 소스 앵커: [openrouter.go](../openrouter.go), [nvidia.go](../nvidia.go), [copilot.go](../copilot.go), [catalog.go](../catalog.go).
...
- 프로바이더 컨텍스트 길이는 `model-metadata` 브랜치의 원시 메타데이터 카탈로그에서 보강돼요. 카탈로그는 `node scripts/update-model-metadata.mjs`와 일일 `.github/workflows/update-model-metadata.yml` 워크플로우에서 공개 OpenRouter 및 NVIDIA 메타데이터 엔드포인트에서 새로고침돼요. Copilot 모델은 하드코딩된 컨텍스트 길이를 사용해요.
...
3. 소스 앵커를 확인하세요:
   ...
   - [server.go](../server.go)에서 선택된 모델 필터링과 요청 전달.
4. 테스트를 확인하세요:
   - `openrouter_test.go`
   - `nvidia_test.go`
   - `copilot_test.go`
   - `catalog_test.go`
   - `server_test.go`의 프로바이더 관련 커버리지
