...
- 모델 그룹 `fast`, `balanced`, `capable`은 `slr/fast`, `slr/balanced`, `slr/capable` 또는 `haiku`/`sonnet`/`opus` 별칭으로 별도 풀을 지정할 수 있어요. 비어있지 않은 그룹은 해당 그룹 내에서만 라우팅하고, 비어있는 그룹은 전체 선택된 목록으로 대체해요.
