...
- Do not silently remove or force-disable a caller's thinking/reasoning configuration to make a request appear compatible. Preserve supported caller effort/settings; do not reintroduce the removed model-config `reasoning_effort` default. Do not infer uniform reasoning support or semantics from provider identity. When a model or target protocol cannot represent a feature, handle it explicitly and document the exact limitation. Never fabricate provider-signed/private reasoning blocks.
...
- Do not amend existing commits, rewrite history, force-push, or discard work. Once the requested work is implemented, documented, and appropriately verified, commit and push it to the current branch's tracking remote as part of the same task; do not wait for another user prompt. Stage only files belonging to that task, use a normal non-force push, and stop to resolve a divergent remote rather than rewriting history. Skip commit/push only when the user explicitly asks to leave the work uncommitted or unpushed, or a concrete external blocker prevents it.
