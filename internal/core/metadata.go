...
func metadataKey(source types.ModelSource, id string) string {
...
func fetchRemoteMetadataCatalog(ctx context.Context, client types.HTTPDoer) ProviderMetadataCatalog {
	...
	req.Header.Set("User-Agent", "sleepyrouter/"+types.Version)
...
func LoadModelMetadataCatalog(ctx context.Context, client types.HTTPDoer) ProviderMetadataCatalog {
	if client == nil {
		metadataCacheMu.Lock()
...
func ModelMetadata(source types.ModelSource, id string, catalog ProviderMetadataCatalog) (ProviderModelMetadata, bool) {
	if catalog == nil {
		catalog = localMetadataCatalog()
	}
