...
func NormalizeOpenRouterModel(model OpenRouterModel, popularityRank *int, catalog ProviderMetadataCatalog) types.SleepyRouterModel {
	rawID := model.ID
	if rawID == "" {
	...
	metadata, _ := ModelMetadata(types.SourceOpenRouter, rawID, catalog)
	contextLength := utils.ParseTokenCount(model.ContextLength)
...
func fetchOpenRouterModels(ctx context.Context, apiKey, category string, client types.HTTPDoer) ([]OpenRouterModel, error) {
	...
	req, err := utils.GetRequest(ctx, endpoint.String(), map[string]string{
	...
	if !utils.IsOK(response) {
		...
		return nil, fmt.Errorf("OpenRouter 모델 목록 요청 실패: %d %s (GET /api/v1/models%s)", response.StatusCode, utils.StatusText(response), suffix)
	}
	var body struct {
...
func ListOpenRouterFreeModels(ctx context.Context, apiKey string, client types.HTTPDoer) ([]types.SleepyRouterModel, error) {
	...
	models := make([]types.SleepyRouterModel, 0)
	for index, model := range allModels {
		...
		models = append(models, NormalizeOpenRouterModel(model, utils.IntPointer(rank), metadata))
	}
