...
func handleHealth(deps routeDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		httperr.WriteJSON(w, 200, map[string]any{"ok": true, "service": "sleepyrouter", "version": version.Version, "uptime": int(time.Since(deps.startTime).Seconds())})
	}
}
...
func handleModels(deps routeDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		httperr.WriteJSON(w, 200, map[string]any{"object": "list", "data": data})
	}
}
...
func handleCountTokens() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		httperr.WriteJSON(w, 200, map[string]any{"input_tokens": handler.EstimateInputTokens(body)})
	}
}
...
func handleNotFound() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		httperr.WriteJSONError(w, 404, fmt.Sprintf("지원하지 않는 엔드포인트예요: %s %s. 사용 가능한 엔드포인트: GET /health, GET /v1/models, POST /v1/chat/completions, POST /anthropic/v1/messages", r.Method, r.URL.Path))
	}
}
