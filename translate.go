...
func ExtractTextContent(content any) string {
	...
	for _, block := range blocks {
		...
		if m, ok := block.(map[string]any); ok {
			if m["type"] == "text" {
				parts = append(parts, stringFromUnknown(m["text"]))
				continue
			}
			typeName := unknownString(m["type"])
			if typeName == "" {
				typeName = "unknown"
			}
...
func imageUrlFromAnthropic(block map[string]any) string {
	...
	if source["type"] == "url" {
		return stringFromUnknown(source["url"])
	}
	if source["type"] == "base64" {
		mediaType := stringFromUnknown(source["media_type"])
		data := stringFromUnknown(source["data"])
		if mediaType != "" && data != "" {
...
func openAIContentFromBlocks(blocks []map[string]any) any {
	...
	for _, block := range blocks {
		if block["type"] == "text" {
			text := stringFromUnknown(block["text"])
			if text != "" {
	...
	if allText {
		...
		for _, part := range parts {
			texts = append(texts, stringFromUnknown(part["text"]))
		}
...
func anthropicMessagesToOpenAI(messages any) []map[string]any {
	...
	for _, raw := range msgs {
		...
		role := stringFromUnknown(msg["role"])
...
func toolsToOpenAI(tools any) []map[string]any {
	...
	for _, raw := range toolList {
		...
		name := stringFromUnknown(tool["name"])
		if name == "" {
			continue
		}
...
func toolChoiceToOpenAI(toolChoice any) any {
	...
	tcType := stringFromUnknown(tc["type"])
	if tcType == "" {
		return nil
	}
	switch tcType {
	...
	case "any":
		return "required"
	case "tool":
		name := stringFromUnknown(tc["name"])
		if name != "" {
...
func systemToText(system any) any {
	...
	for _, raw := range blocks {
		if block, ok := raw.(map[string]any); ok {
			parts = append(parts, stringFromUnknown(block["text"]))
		}
	}
...
func contentFromOpenAI(content any) string {
	...
	for _, part := range parts {
		...
		if m, ok := part.(map[string]any); ok {
			if m["type"] == "text" {
				result = append(result, stringFromUnknown(m["text"]))
				continue
			}
...
func OpenAIToAnthropic(response map[string]any, fallbackModel string) map[string]any {
	...
	inputTokens := 0
	outputTokens := 0
	if v := numberValue(usage["prompt_tokens"]); v != nil {
		inputTokens = *v
	} else if v := numberValue(usage["input_tokens"]); v != nil {
		inputTokens = *v
	}
	if v := numberValue(usage["completion_tokens"]); v != nil {
		outputTokens = *v
	} else if v := numberValue(usage["output_tokens"]); v != nil {
		outputTokens = *v
	}
