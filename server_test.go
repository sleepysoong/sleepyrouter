...
func withTestServerHandler(store *ConfigStore, client HTTPDoer, env Environment, fn func(handler http.Handler)) {
...
func TestServer_ReturnsSelectedModels(t *testing.T) {
	store, cleanup := tempServerStore(t)
	defer cleanup()
	withTestServerHandler(store, nil, Environment{"OPENROUTER_API_KEY": "key"}, func(handler http.Handler) {
...
func TestServer_NonFreeModelRejected(t *testing.T) {
	...
	withTestServerHandler(store, mock, Environment{"OPENROUTER_API_KEY": "key"}, func(handler http.Handler) {
		reqBody, _ := json.Marshal(map[string]any{
			"model":    "paid/model",
...
func TestServer_RoutesNVIDIAAnthropic(t *testing.T) {
	...
	withTestServerHandler(store, mock, Environment{"NVIDIA_API_KEY": "nkey"}, func(handler http.Handler) {
		reqBody, _ := json.Marshal(map[string]any{
			"model":    "auto",
			"messages": []any{map[string]any{"role": "user", "content": "hi"}},
			"max_tokens": 100,
