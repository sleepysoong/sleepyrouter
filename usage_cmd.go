...
func filterUsageLogs(logs []UsageLogEntry, date string, week int) []UsageLogEntry {
	if date == "" && week == 0 {
		return logs
	}
	result := make([]UsageLogEntry, 0, len(logs))
...
func aggregateUsage(logs []UsageLogEntry) []usageAggregateRow {
