...
- Implementation: `server.go`, `sse.go`, `translate.go`
- Tests: `server_test.go`, `translate_test.go`
...
- Per-mode model names are handled as local routing aliases before upstream forwarding, so clients can use `sleepyrouter/fast`, `sleepyrouter/balanced`, `sleepyrouter/capable`, or the `haiku`/`sonnet`/`opus` aliases without the upstream provider exposing those IDs.
...
3. Add or adjust `server_test.go` and `translate_test.go` coverage.
