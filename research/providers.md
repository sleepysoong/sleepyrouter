...
- Implementation: `src/providers/openrouter.ts`, `src/providers/nvidia.ts`, `src/providers/copilot.ts`, `src/providers/catalog.ts` (multi-provider aggregation entry point), `src/providers/types.ts`
- Tests: `test/openrouter.test.ts`, `test/nvidia.test.ts`, `test/copilot.test.ts`, `test/catalog.test.ts`, provider-related coverage in `test/server.test.ts`, `test/model-command.test.ts`, and `test/probe.test.ts`
...
- GitHub Copilot provider implements a 2-step token exchange flow (GitHub Token -> Copilot Session Token via `api.github.com/copilot_internal/v2/token`) and caches the short-lived session token. Since there is no public `/models` endpoint for Copilot, the provider hardcodes a known list of free models (e.g., gpt-4o, claude-sonnet-4).
