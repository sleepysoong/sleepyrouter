...
func ParseStream(ctx context.Context, scanner *sse.Scanner, out chan<- provider.StreamChunk) {
	...
	for {
		...
		if delta.ReasoningContent != "" {
			if !provider.TrySend(ctx, out, provider.StreamChunk{Type: provider.ChunkReasoning, Text: delta.ReasoningContent, Metadata: deltaReasoningMeta(delta.ThoughtSignature, delta.Signature, delta.ExtraContent)}) {
				return
			}
		} else if delta.Reasoning != "" {
			if !provider.TrySend(ctx, out, provider.StreamChunk{Type: provider.ChunkReasoning, Text: delta.Reasoning, Metadata: deltaReasoningMeta(delta.ThoughtSignature, delta.Signature, delta.ExtraContent)}) {
				return
			}
		}
		...
		for _, tc := range delta.ToolCalls {
			...
			// Capture thought-signature fields (Gemini/OpenRouter style
			// continuity) so they can be re-emitted on the wire.
			if sig := toolCallSignature(tc.ThoughtSignature, tc.Signature, tc.ExtraFields, tc.Function.ThoughtSignature, tc.Function.Signature); sig != "" {
				active.thoughtSignature = sig
			}

			if active.id != "" && active.name != "" && !active.started {
				var meta map[string]any
				if active.thoughtSignature != "" {
					meta = map[string]any{"thoughtSignature": active.thoughtSignature}
				}
				if !provider.TrySend(ctx, out, provider.StreamChunk{
...
func ParseResponse(body []byte) (*provider.GenerateResult, error) {
	...
	if len(resp.Choices) > 0 {
		...
		if sig := deltaReasoningMeta(choice.Message.ThoughtSignature, choice.Message.Signature, choice.Message.ExtraContent); sig != nil {
			providerMeta["thoughtSignature"] = sig["thoughtSignature"]
		}
		...
		for _, tc := range choice.Message.ToolCalls {
			...
			if sig := toolCallSignature(tc.ThoughtSignature, tc.Signature, tc.ExtraFields, tc.Function.ThoughtSignature, tc.Function.Signature); sig != "" {
				call.Metadata = map[string]any{"thoughtSignature": sig}
			}
...
// toolCallSignature extracts the thought-signature from a parsed stream tool
// call, in the precedence order the legacy stream translator established:
// tool-call thought_signature, tool-call signature, function-level
// thought_signature, function-level signature, then extra_fields.
func toolCallSignature(tcThoughtSig, tcSig string, extraFields map[string]any, fnThoughtSig, fnSig string) string {
	if tcThoughtSig != "" {
		return tcThoughtSig
	}
	if tcSig != "" {
	...
	if sig, ok := extraFields["thought_signature"].(string); ok {
		return sig
	}
	return ""
}

// deltaReasoningMeta builds the Metadata map for a reasoning chunk so the
// downstream emitter can re-emit the signature when it closes a thinking
// block. Sources: delta thought_signature, delta signature, then
// extra_content.google.thought_signature (Gemini stream shape).
func deltaReasoningMeta(thoughtSig, sig string, extraContent map[string]any) map[string]any {
	contentSig := ""
	...
	switch {
	case thoughtSig != "":
		return map[string]any{"thoughtSignature": thoughtSig}
	case sig != "":
		return map[string]any{"thoughtSignature": sig}
	case contentSig != "":
		return map[string]any{"thoughtSignature": contentSig}
	}
	return nil
}
