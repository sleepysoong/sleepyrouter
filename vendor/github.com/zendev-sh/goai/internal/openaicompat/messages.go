...
// mergeMaps deep-merges src into dst. Existing dst values win on scalar
// conflicts; nested maps recurse so maps merge key-by-key.
...
func ConvertMessages(msgs []provider.Message, system string, includeReasoningContent ...bool) []map[string]any {
	...
	for _, msg := range msgs {
		...
		// Per-message ProviderOptions overlay: carry any extra wire keys
		// (Google/Gemini thinking state, per-message metadata, ...) without
		// ever clobbering the canonical fields role/content/tool_calls/
		// reasoning_content, which are (re)assigned below.
		if len(msg.ProviderOptions) > 0 {
			mergeMaps(m, msg.ProviderOptions)
		}
		...
		for _, part := range msg.Content {
			...
			case provider.PartToolCall:
				...
				// Carry per-part ProviderOptions (thought signature, extra
				// fields, index) onto the tool_call and its function so
				// Gemini-style continuation fields survive the round trip.
				if len(part.ProviderOptions) > 0 {
					mergeMaps(fn, part.ProviderOptions)
					mergeMaps(tc, part.ProviderOptions)
				}
